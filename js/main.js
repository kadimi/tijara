jQuery(document).ready(function ($) {
    ...
    var woocommerce_installed =  window.hasOwnProperty('woocommerce_params');
    var woocommerce_cart_timeout;
    ...
    $('span.generated', 'footer').remove();
    $('.no-collapse .generated', '#sidebar').remove();

    // Toggle visibility of sidebar contents when titles are clicked 
    $("#sidebar :not(.no-collapse) .widget-title").each(function () {
    ...
    if (woocommerce_installed && woocommerce_params.review_rating_required === 'yes') {
        $('body').on('submit', '#commentform', function () {
            var $ratings_unfilled = $(this)
                .find("[name^='rating_']")
                .filter(function () {
                    return !($.isNumeric($(this).val()) && $(this).val().match(/^[1-5]$/));
                });
            if ($ratings_unfilled.length) {
                window.alert(woocommerce_params.i18n_required_rating_text);
                return false;
            }
        });
    }
